...
function createRouter(init) {
  ...
  async function handleLoaders(
    ...
    if (redirect5) {
      if (redirect5.idx >= matchesToLoad.length) {
        let fetcherKey =
          revalidatingFetchers[redirect5.idx - matchesToLoad.length].key;
  ...
  async function startRedirectNavigation(state2, redirect5, _temp2) {
    ...
    if (
      redirectPreserveMethodStatusCodes.has(redirect5.status) &&
      activeSubmission &&
...
async function handleLoader({
  ...
  if ((0, import_responses3.isDeferredData)(result)) {
    if (
      result.init &&
      (0, import_responses3.isRedirectStatusCode)(result.init.status || 200)
    ) {
      return (0, import_responses3.redirect)(
        new Headers(result.init.headers).get('Location'),
        result.init
      );
    }
    const body = (0, import_responses3.createDeferredReadableStream)(
      result,
  ...
  return (0, import_responses3.isResponse)(result)
    ? result
    : (0, import_responses3.json)(result);
}
...
function _errorHandler({ error, handler: handleError }) {
  ...
  return (0, import_responses3.json)(
